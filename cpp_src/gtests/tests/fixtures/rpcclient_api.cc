...
void RPCClientTestApi::StartServer(const std::string& addr, Error errOnLogin) {
	...
	}
	{
		...
		if (it != realServers_.end()) {
			...
			it->second.serverThread.reset(new std::thread([&it] {
				int res = it->second.server->Start();
				assert(res == EXIT_SUCCESS);
				(void)res;
			}));
			while (!it->second.server->IsReady()) {
				std::this_thread::sleep_for(std::chrono::milliseconds(1));
			}
			return;
		}
...
void RPCClientTestApi::AwaitConnect(client::CoroReindexer& rx, ev::dynamic_loop& loop) {
	Error err;
	size_t cnt = 0;
	do {
		err = rx.Status();
		loop.sleep(std::chrono::milliseconds(50));
		std::cout << "Cnt: " << cnt << ", err: " << (err.ok() ? "OK" : err.what()) << std::endl;
	} while (!err.ok() && ++cnt < 5);
	ASSERT_TRUE(err.ok()) << err.what();
}
